...
import conduit.handler.NewArticleDto
...
interface OldRepo {
    ...
    fun createArticle(newArticle: NewArticleDto, authorEmail: Email): ArticleDto
    fun getArticlesFeed(email: Email, offset: Int, limit: Int): MultipleArticles
    fun getTags(): List<ArticleTag>
    fun createArticleComment(newComment: NewComment, slug: ArticleSlug, currentUserEmail: Email): CommentDto
    fun getArticleComments(slug: ArticleSlug, currentUserEmail: Email?): List<CommentDto>
    fun createArticleFavorite(slug: ArticleSlug, currentUserEmail: Email): ArticleDto
    fun deleteArticleFavorite(slug: ArticleSlug, currentUserEmail: Email): ArticleDto
    fun deleteArticleComment(commentId: Int)
    fun deleteArticle(slug: ArticleSlug, currentUserEmail: Email)
    fun getArticle(slug: ArticleSlug, currentUserEmail: Email?): ArticleDto
    fun updateArticle(slug: ArticleSlug, updateArticleDto: UpdateArticle, currentUserEmail: Email): ArticleDto
    fun getArticles(
...
class OldRepoImpl(private val database: Database) : OldRepo {
    ...
    override fun createArticle(newArticle: NewArticleDto, authorEmail: Email): ArticleDto =
        transaction(database) {
        ...
        val resultArticles = articles.map {
            ArticleDto(
                ...
                articleAuthorProfiles[it[Articles.id]]
                    ?: throw Error("ArticleDto ${it[Articles.id]} doesn't have an author.")
            )
        }
    ...
    override fun createArticleComment(newComment: NewComment, slug: ArticleSlug, currentUserEmail: Email): CommentDto =
        transaction(database) {
            ...
            val article = Articles.select { Articles.slug eq slug.value }.singleOrNull() ?: throw HttpException(
                Status.NOT_FOUND,
                "ArticleDto with slug [$slug] not found."
            )
            ...
            Comments.select { Comments.id eq commentId }.single()
                .commentDto(Profile(authorUser.username, authorUser.bio, authorUser.image, false))
        }
    ...
    override fun getArticleComments(slug: ArticleSlug, currentUserEmail: Email?): List<CommentDto> =
        transaction(database) {
    ...
    override fun createArticleFavorite(slug: ArticleSlug, currentUserEmail: Email): ArticleDto =
        transaction(database) {
    ...
    override fun deleteArticleFavorite(slug: ArticleSlug, currentUserEmail: Email): ArticleDto =
        transaction(database) {
            val currentUser = getUser(byEmail(currentUserEmail)) ?: throw UserNotFoundException(currentUserEmail.value)
            val article = Articles.select { Articles.slug eq slug.value }.firstOrNull() ?: throw HttpException(
                Status.NOT_FOUND,
    ...
    override fun getArticle(slug: ArticleSlug, currentUserEmail: Email?): ArticleDto =
        transaction(database) {
            val article = Articles.select { Articles.slug eq slug.value }.firstOrNull() ?: throw HttpException(
                Status.NOT_FOUND,
                "ArticleDto with slug [$slug] not found."
            ...
            val favorited = if (currentUserEmail != null) {
                val currentUser =
    ...
    override fun updateArticle(slug: ArticleSlug, updateArticleDto: UpdateArticle, currentUserEmail: Email): ArticleDto =
        transaction(database) {
            val currentUser =
                getUser(byEmail(currentUserEmail)) ?: throw HttpException(Status.NOT_FOUND, "User not found.")
...
fun ResultRow.toArticleDto(author: Profile, tags: List<ArticleTag>, favorited: Boolean, favoritesCount: Int) = ArticleDto(
...
fun ResultRow.commentDto(author: Profile) = CommentDto(
